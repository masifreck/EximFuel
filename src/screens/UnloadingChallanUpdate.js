...
const unloadingDetails=Unloading?.date || {};
// console.log('Unloading data received:', unloadingDetails ,finishGoodsData);
  // calander==================
  const [selectedStartDate, setSelectedStartDate] = useState(unloadingDetails?.UnloadDate? unloadingDetails.UnloadDate.split('T')[0] : '');
...
const [ChallanNo, setChallanNo] = useState(finishGoodsData?.ChallanNo || '');



const [Id, setId] = useState(unloadingDetails.Id || '');

  const [UnloadDate, setUnloadDate] = useState(unloadingDetails?.UnloadDate? unloadingDetails.UnloadDate.split('T')[0] : '');
  const [UnloadGrossWt, setUnloadGrossWt] = useState(
  unloadingDetails?.UnloadGrossWt?.toString() || '0.00'
);
const [UnloadTareWt, setUnloadTareWt] = useState(
  unloadingDetails?.UnloadTareWt?.toString() || '0.00'
);
const [UnloadWt, setUnloadWt] = useState(
  unloadingDetails?.UnloadWt?.toString() || '0.00'
);
const [GRNNumber, setGRNNumber] = useState(
  unloadingDetails?.GRNNumber?.toString() || ''
);
const [GPSReceived, setGPSReceived] = useState(
  unloadingDetails?.IsGpsReceived === 'True' ? '1' : '0'
);
...
const registertheVehicle = () => {
 if(!Id){
  setErrorMessage('Invalid Unloading ID');
  setShowAlert(true);
  return;
 }

const postData = { Id: Id, GPSReceived: GPSReceived, UnloadDate: convselectedStartDate, UnloadGrossWt: UnloadGrossWt, UnloadTareWt: UnloadTareWt, UnloadWt: UnloadWt, GRNNumber: GRNNumber, };
