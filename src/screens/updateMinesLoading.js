...
const [Id, setId] = useState('');   
  useEffect(() => {
    // ✅ Pre-fill from rowData if available
    if (route.params?.rowData) {
      const data = route.params.rowData;
        setId(data.Id || '');
      setTpScan(data.TpNo || '');
      setChallan(data.ChallanNo || '');
      setTruckNo(data.VehicleNo || '');
      setCash(data.Cash?.toString() || '');
      setAmount(data.Amount?.toString() || '');
      setHsdAmt(data.HsdAmount?.toString() || '');
      setPumpName(data.PumpName || '');
      setSlipNo(data.SlipNo || '');
    }

    // ✅ Still allow scanned values override
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        'https://Exim.Tranzol.com/api/LoadingChallan/UpdateLoadingRowMetrial',
        {
          method: 'POST',
          headers: {
      ...
      if (result?.apiResult?.StatusCode === 0) {
        Alert.alert('Success', 'Loading row material update successfully!');
        navigation.goBack();
      } else {
...
export default UpdateMinesLoading;
